...
export class ProductFormComponent {
	...
	async renderData(idProduct) {
		...
		if (idProduct) {
			loadFormData = await this.loadProduct(idProduct);
		}
	...
	renderCategoryList(categories) {
		const options = categories.reduce((options, { title: titleCategory, subcategories }) => {
	...
	renderStatusList(statusList) {
		...
		this.subElements.productForm.status.append(
			...statusList.map(({ text, value }) => new Option(text, value)),
		);
	}
	...
	renderNewElementToSortableList = ({ source, url }) => {
		this.subElements.sortableList.append(
	...
	dispatchEvent(formData) {
		const event = formData.id ? 'product-updated' : 'product-saved';
	...
	getFormData() {
		...
		const imagesElementArray = Array.from(
			this.subElements.sortableList.querySelectorAll('.sortable-table__cell-img'),
		);
		return {
	...
	removeEventListeners() {
		...
		this.subElements.sortableList.removeEventListener('click', this.onRemoveImage);
	}
