...
export class SortableList {
	...
	dragStart(item, { clientX, clientY }) {
		...
		this.placeholderElem = HTMLBulder.getElementFromString(
			`<div class="${this.placeholderClass}"></div>`,
		);
	...
	moving(callback) {
		...
		const { left: leftHolder, top: topHolder } = this.placeholderElem.getBoundingClientRect();
	...
	onPointerMove = event => {
		...
		for (const item of children) {
			if (this.draggingElemement == item) {
				return;
			}
			...
			if (
				...
				if (
					this.placeholderElem.getBoundingClientRect().bottom <
