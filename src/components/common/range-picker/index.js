...
export class RangePicker {
	...
	renderHighlight() {
		Array.from(this.element.querySelectorAll('.rangepicker__cell')).forEach(buttonDay => {
	...
	renderFromToDate() {
		this.subElements.from.textContent = RangePicker.getFormatDate(this.selected.from);
		this.subElements.to.textContent = RangePicker.getFormatDate(this.selected.to);
	}
